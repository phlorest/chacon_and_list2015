...
- Re-create the CLDF dataset:
  ```shell
  cldfbench makecldf --glottolog-version v5.2 cldfbench_chacon_and_list2015.py --with-zenodo --with-cldfreadme
  ```
...
- Run Phlorest-specific checks on the data:
  ```shell
  phlorest check --with-R cldfbench_chacon_and_list2015.py
  ```
...
- Create the release instruction:
  ```shell
  phlorest release cldfbench_chacon_and_list2015.py vX.Y
  ```
